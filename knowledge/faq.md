...
## 8. Delay/Jeda antara Pergerakan Kendaraan Asli dengan Tampilan di Aplikasi

### Pertanyaan:
*"Apakah GPS IntelliTrac bisa memantau posisi kendaraan secara real-time? Seberapa akurat dan cepat update lokasinya? Apakah ada delay antara posisi kendaraan sebenarnya dengan yang tampil di aplikasi/dashboard?"*

### Panduan Jawaban untuk Mintel:
1. **Interval Pengiriman Bukan Jaminan Tanpa Delay**: interval pengiriman posisi saat mesin menyala (15 detik / perubahan heading 15 derajat / event triggered, lihat bagian 6 di atas) itu soal seberapa sering **perangkat** mengirim update, bukan jaminan kecepatan data itu sampai dan tampil di layar user. JANGAN menjawab seolah update di aplikasi pasti muncul persis sekian detik setelah kejadian.
2. **Faktor yang Memengaruhi Delay Sesungguhnya**: waktu tempuh data dari perangkat ke server lalu ke aplikasi/dashboard user bergantung pada kualitas jaringan seluler di lokasi kendaraan saat itu, antara lain kekuatan sinyal 4G/2G di titik tersebut, lokasi & kepadatan BTS provider (Telkomsel) di wilayah tersebut, serta kondisi bandwidth/kepadatan jaringan provider di wilayah tersebut. JANGAN memastikan angka delay pasti dalam detik, karena ini bervariasi tergantung faktor-faktor di atas dan tidak sepenuhnya di bawah kendali IntelliTrac.
3. **Mitigasi untuk Wilayah dengan Sinyal Seluler Lemah, Tapi Ada WiFi**: kalau lokasi operasional armada minim sinyal seluler tapi tersedia infrastruktur WiFi (misal gudang, kantor, area tambang yang sudah terpasang WiFi), JC261 (fitur Wi-Fi Connectivity) dan VT-45 (fitur Wi-Fi Module) bisa memakai WiFi sebagai jalur alternatif pengiriman data tracking, bukan cuma sinyal seluler. SSID WiFi lokasi tersebut perlu didaftarkan dulu ke sistem sebelum bisa dipakai. VT-45 Lite dan JC450 dukungan WiFi-nya belum terdokumentasi di KB ini, JANGAN memastikan ketersediaannya untuk dua tipe ini, arahkan ke Sales/teknisi kalau customer menanyakan spesifik untuk tipe tersebut.
4. **Arahkan ke Sales/Teknisi** untuk armada yang rutin beroperasi di area bersinyal lemah, agar kebutuhan mitigasi (WiFi alternatif, penyesuaian interval, dsb) bisa ditinjau lebih lanjut.
