...
Saat kendaraan masuk ke area tanpa sinyal seluler (namun sinyal GPS tetap ada), perangkat tetap menerima sinyal satelit GPS dan terus mencatat titik koordinat, status mesin, kecepatan, serta data sensor ke memori internal. Begitu sinyal seluler kembali, seluruh rekaman offline itu **terunggah secara bertahap** ke server, sehingga riwayat perjalanan dan laporan armada tetap lengkap, selama kapasitas memori internal masih cukup menampung durasi offline tersebut.

Ada satu hal yang perlu dibedakan di sini: hasil di atas hanya berlaku kalau sinyal GPS-nya tetap ada. Kalau sinyal GPS-nya juga ikut hilang (misalnya di basement, terowongan, atau gudang tertutup), posisi memang tidak bisa dihitung sama sekali selama di dalam, sehingga **tidak ada riwayat yang bisa dibuffer untuk periode itu**. Begitu kembali ke area terbuka, perangkat langsung melanjutkan pelacakan dari posisi terkini, tanpa ada data lampau yang di-backfill.

| Kondisi | Sinyal GPS | Sinyal Seluler | Hasil |
|---|---|---|---|
| Perkebunan pedalaman, tambang terpencil, jalur laut/hutan | Ada | Tidak ada | Posisi tetap terekam ke memori internal, lalu tersinkron **secara bertahap** saat sinyal kembali |
| Wilayah tertutup/terhalang bangunan tinggi, namun BTS provider masih terjangkau | Tidak ada | Ada | Event data (mesin nyala, pintu buka, dll) tetap terkirim real-time tanpa posisi; begitu GPS kembali, langsung lanjut ke posisi terkini tanpa posisi lampau |
| Basement, terowongan, gudang tertutup, garasi bertingkat | Tidak ada | Tidak ada | Posisi tidak bisa dihitung selama di dalam, tidak ada yang dibuffer; begitu kembali ke area terbuka, langsung lanjut ke posisi terkini tanpa data lampau |

> **Catatan untuk Mintel**: jangan menjanjikan riwayat "utuh 100%" atau "tanpa ada data yang terlewat" tanpa syarat. Klaim itu hanya berlaku untuk baris pertama tabel (sinyal seluler hilang, GPS tetap ada), dan bahkan itu pun tetap dibatasi kapasitas buffer memory (kalau kendaraan offline sangat lama, data terlama berpotensi tertimpa). Untuk baris kedua dan ketiga (sinyal GPS ikut hilang), secara desain tidak ada data lampau yang di-backfill, jadi jangan menjanjikan riwayat akan "menyusul" setelah sinyal kembali. Jangan juga memakai basement sebagai contoh blank spot seluler biasa, karena di sana sinyal GPS-nya ikut terhalang. Untuk armada yang rutin beroperasi di area dengan pola sinyal seperti ini, arahkan ke Sales Executive supaya kapasitas buffer dan konfigurasi intervalnya bisa disesuaikan.
>
> **Khusus baris kedua (sinyal seluler ada, GPS tidak ada)**: ini yang paling sering bikin customer khawatir secara tidak langsung, misalnya bertanya "kalau pintu box dibuka saat di parkiran bawah tanah, apa saya masih kebagian notifikasinya?". JANGAN menjawab kabur dengan hanya bilang "posisi tidak terkirim" tanpa menegaskan nasib event data-nya. **Tegaskan proaktif** bahwa notifikasi/event data (mesin nyala, pintu buka, overspeeding) tetap terkirim real-time meski GPS tidak dapat sinyal, jadi customer tidak perlu khawatir kehilangan notifikasi kejadian penting. Yang tertunda hanya update posisi/koordinatnya. Rincian lengkap tiap kombinasi sinyal ada di `faq.md` bagian Kondisi Blank Spot.
